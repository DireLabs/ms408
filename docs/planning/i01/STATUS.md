...
| T1.3 bulk annotation | ✅ done 2026-07-06 — **G2 awaiting Tim** | All 227 pages annotated (Sonnet 4.6, strict tool use), $3.70 (well under $100 cap). QA: Fable 5 false-positive-refuses manuscript annotation → Opus 4.8 fallback (independent reviewer); cross-model drift 29.7% overall / 27% critical, concentrated in subjective fields (text_image_relationship 46%, damage/marginalia 35%). **Above provisional L31 thresholds — batch-1 tuning decision for Tim (G2).** results/annotations/. |
