...
| T0.3 validation harness | 🔄 in progress | Specs done for both generators (docs/planning/i01/specs/). **H2 Naibbe reimplemented + validated** (golden decrypt byte-exact; stats within author spread). **H4 controls acquired**: Latin (Vulgate + Macer Floridus), Italian (Decameron, Hamilton 90 orthography), German (ReM + ReF incl. Ulmer Wundarznei late-15th-c. medical prose), Hebrew (Mishneh Torah, PD version) — 13 files, MANIFEST.json w/ sha256+licenses. Remaining: H3 self-citation generator, H4 normalization pass, scoring API, benchmark report. |
