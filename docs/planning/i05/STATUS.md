...
| E14b | K/V robustness for E14 | ✅ done → **E14 CONFIRMED + A/B difference surfaced** | [C] Swept K{8,12,20}×V{40,60,100}=9 cells. Core robust: real langs calibrate all cells; VMS-A far below real in **9/9** cells, VMS-B in 8/9 (one K8/V100 outlier). ALSO: **VMS-B > VMS-A in 7/9 cells** — a consistent, parameter-stable A/B difference. What E14 saw as a near-threshold straddle is REAL: the B system (biol./recipe) carries more word-class structure than A (herbal), both far below real language. Supports Tim's "A and B differ" hypothesis (in degree). Dedicated A-vs-B study now warranted. |
