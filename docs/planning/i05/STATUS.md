...
| E14 | Distributional word-class (POS) induction | P2 | ✅ done (null-corrected) → **VMS weak word-class structure** | [C] Built null-corrected from the start (k-means on context vectors → adjacent-class NMI, z vs order-shuffle+recluster null). CALIBRATED (real langs z≈14.8). **VMS shows only WEAK word-class transition structure — A z=1.9 (13% of real), B z=2.8 (19%), both far below real language.** The A/B threshold-straddle is NOT read as "different processes" (both near floor; needs dedicated A-vs-B replication). Converges with E13c: VMS word-LEVEL distributional structure is weak/different from natural language despite strong character + morphology structure. SCOPE (L7): surface distributional classes only. |
| E14b | K/V robustness for E14 | ✅ done → **E14 CONFIRMED**; A/B difference surfaced (later corrected by E17) | [C] Swept K{8,12,20}×V{40,60,100}=9 cells. Core robust: VMS-A far below real in **9/9** cells, VMS-B in 8/9. Surfaced VMS-B > VMS-A in 7/9 cells — **but E17 shows that A/B difference is a CONTENT confound, not dialect.** |
| E17 | Dedicated A-vs-B contrast (content-controlled) | ✅ done → **A/B difference is a CONTENT confound, NOT dialect** | [C] The herbal section has both dialects (Herbal-A 7470, Herbal-B 3305 tokens) — the only content-controlled A/B comparison. Result: B>A word-class z holds GLOBALLY (B−A +1.79) but VANISHES/reverses within herbal (B−A −1.24; herbal-A 1.09 ≥ herbal-B −0.15). So the E14b A/B difference was driven by SECTION/subject matter (B covers biol./recipe), not the Currier dialect. **Content-controlled, NO evidence A and B differ in mid-level word-class structure** → withdraws "A and B are different generative processes" from this probe. CAVEAT: Herbal-B small ⇒ within-herbal contrast underpowered; robust claim is "global difference is content-confounded". Corrects E14b, like E12 corrected E10. |
